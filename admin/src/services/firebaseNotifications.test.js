...
jest.mock('./adminApi', () => ({
  __esModule: true,
  default: { registerNotificationToken: jest.fn() },
}));
...
beforeEach(() => {
  jest.resetModules();
  localStorage.clear();
  localStorage.setItem('1app_admin_token', 'authenticated-session');
  ...
  registerToken = require('./adminApi').default.registerNotificationToken;
...
test('foreground messages display a Firebase browser notification', async () => {
...
test('session cleanup cancels pending old tokens and preserves the new account listener', async () => {
  ...
  localStorage.setItem('1app_admin_token', 'new-authenticated-session');
  await expect(enableBrowserNotifications({ requestPermission: false })).resolves.toBe(true);
