...
const buildServiceData = async (
    ...
    const data = {
        ...
        actualPrice: Math.floor(parseFloat(body.actualPrice) || 0),
        discountPercentage: Math.floor(parseFloat(body.discountPercentage) || 0),
        offerPrice: Math.floor(parseFloat(body.offerPrice) || 0),
        variants: parseJSON(body.variants, []).map(v => ({
            ...v,
            actualPrice: Math.floor(parseFloat(v.actualPrice) || 0),
            discountPercentage: Math.floor(parseFloat(v.discountPercentage) || 0),
            offerPrice: Math.floor(parseFloat(v.offerPrice) || 0),
            duration: Math.floor(parseFloat(v.duration) || 0),
        })),
        addons: parseJSON(body.addons, []).map(a => ({
            ...a,
            price: Math.floor(parseFloat(a.price) || 0),
        })),
